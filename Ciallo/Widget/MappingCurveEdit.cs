...
public partial class MappingCurveEdit : Control
{
    ...
    public void UsePreset(PresetId presetId)
    {
        ...
        switch (presetId)
        {
            ...
            case PresetId.EaseIn:
                _curve.AddPoint(new Vector2(minX, minY));
                _curve.AddPoint(new Vector2(maxX, maxY), _curve.GetValueRange() / _curve.GetDomainRange() * 1.4f, 0);
                break;

            case PresetId.EaseOut:
                _curve.AddPoint(new Vector2(minX, minY), 0, _curve.GetValueRange() / _curve.GetDomainRange() * 1.4f);
                _curve.AddPoint(new Vector2(maxX, maxY));
                break;
    ...
    public override void _GuiInput(InputEvent @event)
    {
        ...
        if (@event is InputEventMouseButton { Pressed: true } mouseButton)
        {
            ...
            if (mouseButton.ButtonIndex == MouseButton.Left)
            {
                ...
                if (_selectedIndex != -1)
                {
                    ...
                    if (_selectedIndex > 0)
                        _initialGrabLeftTangent = _curve.GetPointLeftTangent(_selectedIndex);
                    if (_selectedIndex < _curve.GetPointCount() - 1)
                        _initialGrabRightTangent = _curve.GetPointRightTangent(_selectedIndex);
                }
                else if (_grabbing == GrabMode.None)
                {
                    ...
                    {
                        newPos.X = Mathf.Snapped(newPos.X - _curve.MinDomain, _curve.GetDomainRange() / _snapCount) + _curve.MinDomain;
        ...
        if (@event is InputEventMouseButton { ButtonIndex: MouseButton.Left, Pressed: false })
        {
            if (_selectedTangentIndex != TangentIndex.None)
            {
                if (_selectedIndex == 0)
                {
                    SetPointRightTangent(_selectedIndex, _curve.GetPointRightTangent(_selectedIndex));
                }
                else if (_selectedIndex == _curve.GetPointCount() - 1)
                {
                    SetPointLeftTangent(_selectedIndex, _curve.GetPointLeftTangent(_selectedIndex));
                }
                else
                {
                    SetPointTangents(_selectedIndex, _curve.GetPointLeftTangent(_selectedIndex), _curve.GetPointRightTangent(_selectedIndex));
                }
                _grabbing = GrabMode.None;
            }
                ...
                if (_selectedIndex != -1)
                {
                    if (_selectedTangentIndex == TangentIndex.None)
                    {
                        ...
                        if (mouseMotion.IsAltPressed())
                        {
                            ...
                            float nextPointOffset = _selectedIndex < _curve.GetPointCount() - 1 ? _curve.GetPointPosition(_selectedIndex + 1).X - 0.00001f : _curve.MaxDomain;
    ...
    {
        QueueRedraw();
        if (_selectedIndex >= _curve.GetPointCount())
            SetSelectedIndex(-1);
    }
    ...
    private void AddPoint(Vector2 pos)
    {
        ...
        _curve.AddPoint(pos);
        SetSelectedIndex(_curve.GetPointCount() - 1);
    }

    private void RemovePoint(int index)
    {
        if (_curve == null || index < 0 || index >= _curve.GetPointCount())
            return;
    ...
    private void SetPointPosition(int index, Vector2 pos)
    {
        ...
        // Note: Undo/redo not supported, so we set the position directly
    ...
    private void SetPointTangents(int index, float left, float right)
    {
        ...
        _curve.SetPointLeftTangent(index, left);
        _curve.SetPointRightTangent(index, right);
        SetSelectedIndex(index);
    }

    private void SetPointLeftTangent(int index, float tangent)
    {
        if (_curve == null || index < 0 || index >= _curve.GetPointCount())
            return;
        ...
        _curve.SetPointLeftTangent(index, tangent);
        SetSelectedIndex(index);
    }

    private void SetPointRightTangent(int index, float tangent)
    {
        if (_curve == null || index < 0 || index >= _curve.GetPointCount())
            return;
        ...
        // Note: Undo/redo not supported
        _curve.SetPointRightTangent(index, tangent);
        SetSelectedIndex(index);
    }

    private void ToggleLinear(int index, TangentIndex tangent)
    {
        if (_curve == null || index < 0 || index >= _curve.GetPointCount() || tangent == TangentIndex.None)
            return;
    ...
    private Vector2 GetTangentViewPos(int index, TangentIndex tangent)
    {
        Vector2 dir;
        if (tangent == TangentIndex.Left)
            dir = -new Vector2(1, _curve.GetPointLeftTangent(index));
        else
            dir = new Vector2(1, _curve.GetPointRightTangent(index));
    ...
    private void PlotCurveAccurate(float step, Color lineColor, Color edgeLineColor)
    {
        float minX = _curve.MinDomain;
        float maxX = _curve.MaxDomain;
        if (_curve.GetPointCount() <= 1)
        {
            float y = _curve.Sample(0);
        ...
        Vector2 lastPoint = _curve.GetPointPosition(_curve.GetPointCount() - 1);
        ...
        for (int i = 1; i < _curve.GetPointCount(); i++)
        {
            ...
            for (int j = 1; j < samples; j++)
            {
                ...
                pos.X = a.X + x;
                pos.Y = _curve.SampleLocalNoCheck(i - 1, x);
    ...
    public override void _Draw()
    {
        ...
        else if (_selectedIndex != -1 && _selectedTangentIndex != TangentIndex.None)
        {
            float width = Size.X - 50;
            textColor.A *= 0.8f;
            float theta = Mathf.RadToDeg(Mathf.Atan(_selectedTangentIndex == TangentIndex.Left ? -_curve.GetPointLeftTangent(_selectedIndex) : _curve.GetPointRightTangent(_selectedIndex)));
        ...
        if (Input.IsKeyPressed(Key.Alt) && _grabbing != GrabMode.None && _selectedTangentIndex == TangentIndex.None)
        {
            ...
            float nextPointOffset = _selectedIndex < _curve.GetPointCount() - 1 ? _curve.GetPointPosition(_selectedIndex + 1).X : _curve.MaxDomain;
